...
from app.db.models import Bet, Product
...
class ProductRepository(CRUDRepository):
    ...
    async def get_id(self, id: int) -> Product:
        async with self.async_session as db:
            query = select(Product).where(Product.id == int(id))
            product = await db.execute(query)
            return product.scalars().first()

    async def get_search(self, search: str) -> list[Product]:
        async with self.async_session as db:
    ...
    async def update(self, product: Product, data: dict) -> None:
        async with self.async_session as db:
    ...
    async def get_bids(self, id: int) -> list[Bet]:
        async with self.async_session as db:
